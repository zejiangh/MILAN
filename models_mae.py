...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 norm_pix_loss=False, use_clip=False):
        super().__init__()
    ...
    def forward_encoder(self, x, mask_ratio, importance=None):
        ...
        #     assert self.use_clip is False
        #     x, mask, ids_restore = self.random_masking(x, mask_ratio)
        # else:
        #     assert self.use_clip is True
        #     x, mask, ids_restore = self.attention_masking(x, mask_ratio, importance)
        x, mask, ids_restore = self.random_masking(x, mask_ratio)
    ...
    def forward_loss(self, imgs, pred, mask, clip_teacher=None):
        if self.use_clip is False:
            """
            imgs: [N, 3, H, W]
            pred: [N, L, p*p*3]
    ...
    def forward(self, imgs, mask_ratio=0.75, clip_teacher=None):
        if self.use_clip:
            clip_teacher, importance = clip_teacher[0], clip_teacher[1]
            importance = importance[:, 0, 1:]
        else:
            clip_teacher, importance = None, None
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio, importance)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(imgs, pred, mask, clip_teacher)
